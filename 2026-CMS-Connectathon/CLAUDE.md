...
├── fhir-resources/        Loadable FHIR R4 JSON — the actual test data, split at
│   ├── durable/           the top level into durable/ (reusable across use cases:
│   └── purpose-built/     orgs, clinicians, plans, payer rules, CDS Hooks) and
│                          purpose-built/ (one episode only, per use case, each
│                          with base/ + interactions/, organized by interaction).
...
|---|---|---|---|
| **Durable** | `fhir-resources/durable/` | Organization, Practitioner, Location, InsurancePlan, Endpoint, payer CRD/DTR rules (PlanDefinition/Library/Questionnaire), CDS Hooks discovery configs (`cds-hooks/`) | Yes — tied to the real-world institution/clinician/plan, not the story |
| **Use-case base** | `fhir-resources/purpose-built/uc01-.../base/` | Patient, Coverage, Consent, **PractitionerRole**, Subscription | No — specific to this patient's episode. PractitionerRole is deliberately NOT durable: the role *pairing* (clinician+org+specialty+location) can vary by use case even when the underlying Practitioner doesn't |
| **Per-interaction** | `fhir-resources/purpose-built/uc01-.../interactions/interaction-0N/` | Encounter, Condition, ServiceRequest, Task, Provenance, etc. | No — tied to that specific interaction in the story |
...
**1. Durable/purpose-built reorg (retroactively logged — the reorg itself predated this entry).** The library was split at the top level into `durable/` (reusable across use cases: organizations, practitioners, locations, endpoints, insurance-plans, payer-rules, and a consolidated `cds-hooks/`) and `purpose-built/<uc>/` (one episode only, each with `base/` + `interactions/`). `fhir-resources/common/` → `durable/`; the three per-use-case folders moved under `purpose-built/`; the three scattered `cds-hooks-discovery-*.json` were consolidated into `durable/cds-hooks/`. New README index at `fhir-resources/README.md` plus `durable/README.md` and `purpose-built/README.md`. Section 2's tier table + structure diagram in this file updated to match (the "Registry" tier is now **Durable**). Historical Session Log entries left as-is — their old paths are accurate for their time.

**2. Full practitioner/role rename (the long-standing "naming wart," now fixed — user chose the full rename over filename-only).** The four UC01 practitioner/role files kept pre-v3.7 real surnames as both filenames AND resource `id`s. Renamed end-to-end — filenames, `id`s, and every reference/`fullUrl`/PUT-url across the durable practitioners, base roles, and all interaction resources/bundles — mapping by verified file *content*, not filename: `pract/role-galloway → -whitfield`, `-lin → -nandakumar`, `-schmalbach → -osei`, `-sollecito → -bellweather`. 32 JSON files rewritten (bundles carried the bulk: 34/39/48/49 token hits) + 8 files physically renamed. HL7v2 needed no change here (confirmed it carries no `pract-*`/`role-*` id tokens). One historical Session Log entry (v2.7) still names `pract-galloway`/`role-galloway` — left untouched as accurate history.

**3. Given-name alignment (new finding, caught by reading content during the rename).** v3.7 had scrubbed family names + emails to the fictional identities but left the **real clinicians' first names** in `Practitioner.name.given` for three of four (Whitfield/Osei/Bellweather still read Thomas/Cecelia/Thomas), while the emails and 30–49 display strings already said Marcus/Renata/Andrew. Aligned `given` to **Marcus / Renata / Andrew** (Priya was already correct) and fixed the stray displays (`"Thomas Mark Whitfield, MD"` → `"Marcus Whitfield, MD"`, plus the Osei/Bellweather equivalents) across the standalone practitioner files, the role files, and all four bundles. Residual-token scan for the old first/middle names came back clean.

**4. Three confirmed-stale artifacts deleted** (all previously believed discarded per the v3.6 log but still present): `purpose-built/uc01-medical-to-dental/uc01-base-resources-bundle.json` (old-style base bundle), `hl7v2/uc01-medical-to-dental/encounter-01/` (stale duplicate of `interaction-01/`; also still carried pre-v3.7 real surnames), and `companion-guides/UC01-interaction-02-companion-guide.md` (superseded by the unified guide; also carried real names). Loadable FHIR JSON count 148 → 147.

**5. HL7v2 cleanup.** Stale `encounter-01/` folder removed (above); `hl7v2/README.md` updated (`fhir-resources/common/` → `durable/`; structure diagram `encounter-01/` → `interaction-01/`; "Encounter #1" → "Interaction 1"). QA notes + `_reference/360x-cow-crosswalk-summary.md` spot-checked — no stale FHIR paths.

**Verification (all clean):** all 147 JSON parse; every UC01 bundle's local references resolve with no duplicate `fullUrl`s; zero remaining old `pract-*`/`role-*` tokens in `fhir-resources/`; the new tokens resolve; the 8 renamed files exist and the old 8 are gone; interaction-bundle entry counts unchanged (I1=35, I2=40, I3=48, I4=51; delta=5); `given` = Marcus/Renata/Andrew/Priya; no stale FHIR paths left in `hl7v2/` or this file's active sections.

**Deferred to the next pass (flagged, untouched):** path/terminology updates inside `companion-guides/UC01-companion-guide.md`, `UC02a-companion-guide.md`, `UC02b-companion-guide.md`, and all `use-cases/**` narrative docs — several still carry old `common/`/`uc01-.../` paths, and the UC01 `use-cases/.../encounters/*.md` files still carry pre-v3.7 real names.

### 2026-07-13 — Claude Code (v5.1) — UC02b Interactions 3–5 built: UC02b now complete end-to-end
Per user plan (`uc02b_i3-i5_build`): built the FHIR resources for UC02b's final three interactions, extended the companion guide to cover all five, and updated the index/session-log docs. This completes UC02b. 11 new JSON files (8 standalone resources + 3 cumulative bundles); no new registry/base resources needed (those were built for I1/I2 in v5.0).

**What UC02b I3–I5 add over UC02a's already-built I3–I5 (the only genuinely new work):** (1) a **second `Procedure`** at the surgical encounter — D6010 immediate endosteal implant alongside the D7210 extraction; (2) the **project's first `Device` resource** (the implant) — UC02b's reason to exist; (3) a **two-line-item** claims-sharing `ExplanationOfBenefit` (D7210 + D6010). Everything else (surgical Encounter, MedicationStatement, ClinicalImpression, CarePlan, completed ServiceRequest, closed Task) mirrors UC02a's structure with UC02b values.

**I3 (`interactions/interaction-03/`)** — surgical visit 2026-07-15 09:00–11:30: `encounter-03-surgical` (`basedOn` referral), `medicationstatement-frank-castle` (none reported, reviewed for surgical risk), `procedure-extraction-tooth30` (D7210), `procedure-implant-tooth30` (D6010, linked to the Device via `Procedure.focalDevice` action `implanted`), and `device-implant-tooth30`. **Device modeling decisions (verified against R4, not assumed):** `type` left **text-only** — searched SNOMED for "endosteal dental implant" and found only a generic "Implant" (40388003) or overly-specific one-piece/two-piece variants that don't cleanly match a screw-type implant, so followed the project's unverifiable-code discipline (same as the periodontal-charting LOINC gap). Placement torque (35 Ncm) carried as a structured `Device.property` (UCUM `N.cm`) plus the note. **No `bodySite`** on Device (R4 has none — site stays on the Procedure); **no `us-core-implantable-device` profile** asserted (synthetic record, no UDI, text-only type — claiming it would be unverifiable conformance, the same over-claim the Encounter.note lesson warns against).

**I4 (`interactions/interaction-04/`)** — close the loop 2026-07-15 11:45–11:46: `clinicalimpression-postop-summary` (names both procedures, references both Procedures in `investigation.item`, names the Device in `summary` since R4 can't reference Device from `investigation.item`), `careplan-postop` (osseointegration + future crown D6065/D6066 as text-only future work; carries the Device via `CarePlan.supportingInfo`), `servicerequest-referral-completed` (same id, `status: completed`, both codes, still no PA), `task-referral-tracking-closed` (same id, `status: completed`, `businessStatus: outcome-final`, `owner` Dr. Maxil, `output` absorbs both Procedures + Device + ClinicalImpression + CarePlan). The Device "travels" to the restorative provider via `CarePlan.supportingInfo` + `Task.output` + the ClinicalImpression summary text — three carriers because R4 gives ClinicalImpression/CarePlan no first-class Device link.

**I5 (`interactions/interaction-05/`)** — `eob-extraction-implant-claims-sharing`: reuses UC02a's canonical `ode-dental-claim` profile (not a second profile — the UC02b narrative's `ODEOralProfessionalEOB` naming discrepancy is flagged in-file), `use: claim`, `outcome: queued`, id `CS-2026-UC02B-001`, **two `item`s** (D7210 + D6010, both tooth #30, serviced 2026-07-15), commercial payer + Coverage, `careTeam` referring=Parker / rendering=Maxil. **No PA reference** in `supportingInfo` (none exists — the whole point vs. UC02a, whose EOB carries a PA number forward); CDT-only, no CPT crosswalk, non-financial (no unitPrice/net/adjudication). The future crown claim (D6065/D6066) is kept explicitly out of scope per the narrative's own "not resolved here" — narrated in the EOB note, not built.

**Bundles:** three self-contained cumulative transaction bundles built by extending the v5.0 I2 bundle (registry + base + prior interactions carried inline): **I3 = 37 entries, I4 = 41, I5 = 42**. Version history preserved in order (referral Task in all three versions referral-sent → appointment-confirmed → outcome-final; ServiceRequest in both active → completed versions), matching UC02a's cumulative-bundle precedent (its I4 bundle likewise carries the Task 3× and the ServiceRequest 2×).

**Companion guide** (`UC02b-companion-guide.md`): status header now all five; added I3/I4/I5 IG rows, receiving-practice stakeholder steps 5–7 (record two Procedures + Device at I3; push summary carrying the Device and close the Task at I4; assemble two-item claim at I5), patient-app milestones (referral-complete now fires at I4; nothing at I3/I5), stub-spec updates, Resource Index tables for I3–I5, and troubleshooting notes for the Device text-only/no-bodySite/no-profile choices and the EOB profile-naming discrepancy.

**Verification (all clean):** all 11 new JSON files parse; scripted cross-reference check confirmed every local reference in the three new bundles resolves (I3, I4, I5) with no duplicate `fullUrl`s; referral-id `REF-2026-UC02B-001` consistent across every version; Task id stable across all three versions with the correct businessStatus progression; `Procedure.focalDevice`, `Device.property`, and the two-item EOB use only real R4 elements (confirmed Device has no `bodySite`/torque element — both handled correctly); dates match the source timeline. **Note on mode:** executed in Claude Code against decisions framed in the plan; the only judgment calls (Device type text-only, no us-core profile) followed existing project discipline rather than inventing new design.

### 2026-07-13 — Claude Code (v5.0) — UC02b Interactions 1–2 built: registry + base + FHIR resources + companion guide, QA'd throughout
Per user plan (scope deliberately limited to UC02b-I1 "Checking Coverage, Choosing a Path" and UC02b-I2 "The Handoff to Oral Surgery"): QA'd both narratives, then built the commercial-payer registry, UC02b base tier, and both interactions' FHIR resources plus the companion guide, verifying throughout. 27 new JSON files, 1 new companion guide, 2 stale files deleted, 3 index docs touched.

**Structural point that makes UC02b different from UC02a (and drove the resource distribution):** there is **no prior authorization** here — commercial coverage, CRD returns a *negative*. So the referral `ServiceRequest` and its tracking `Task` are created in **I1** (not deferred to I2 as in UC02a), and `Task.focus` points at the referral itself from the start (cleaner than UC02a, which had to focus on a DTR `QuestionnaireResponse` since no referral existed yet). No DTR `Questionnaire`/`QuestionnaireResponse` and no PAS `Claim`/`ClaimResponse` anywhere in UC02b — by design.

**QA of narratives (Step 1):** both I1/I2 writeups verified against the Appendix B timeline (exam 10:00; benefit verify 10:30; CRD no-PA 10:32; referral 11:00; received/scheduled 11:30; AppointmentResponse 11:35; consult 2026-07-15) — no content errors. **Reconciled the v4.7-rename duplicates for the I1/I2 pair:** deleted the two stale old-named files (`uc02b-interaction-01-benefit-verification-referral-send.md`, `uc02b-interaction-02-referral-receipt-scheduling.md`) — confirmed identical bodies to the canonical new-named `uc02b-i1-…`/`uc02b-i2-…` files, only the pre-v4.6 H1 titles differed. Refreshed the interactions `README.md` build-status note (was stale — claimed all 10 UC02 interactions were narrative-only, but UC02a is fully built) and added a UC02 section to `companion-guides/README.md` (neither UC02a nor UC02b guide had been indexed there).

**Registry (Step 2, `fhir-resources/common/`):** `org-commercial-dental-ppo` (synthetic placeholder payer — no real commercial payer named, deliberate, unlike UC02a's real DentaQuest), `insplan-commercial-dental-ppo` (annual max $2,000, basic 80% / major-implant 50%, implant waiting period waived), `endpoint-commercial-dental-ppo`, `cds-hooks-discovery-commercial` (order-sign returning a no-PA/benefits-active card), plus `payer-rules/plan-definitions/plandef-commercial-noauth-rule` + `payer-rules/libraries/lib-commercial-noauth-logic` (open question C resolved: built the full PlanDefinition+Library parallel to UC02a, not just the discovery doc — gives the payer role real files to load; the no-PA rule launches no DTR).

**Base (Step 3, `fhir-resources/uc02b-commercial-implant/base/`):** UC02b-specific `patient-frank-castle` (open question F resolved: UC02b-specific, commercial member ID `COMM-TX-00819472`, Medicaid ID dropped since this scenario is commercial-only), `coverage-frank-castle-commercial`, both PractitionerRoles (Dr. Maxil's note references commercial-network participation, not Medicaid), and a Subscription that drops the "PA approved" milestone UC02a had.

**Interactions (Steps 4–5):** I1 = exam Encounter, 2 Conditions (K04.7, K08.89), 2 Observations, 2 DiagnosticReports (periodontal charting text-only — same named LOINC gap carried from UC02a, not re-litigated), DocumentReference, ImagingStudy (support-a-pull pointer), the referral `ServiceRequest` (`ode-dental-to-dental-referral`, `REF-2026-UC02B-001`, no PA number, both D7210 and D6010 options carried, `insurance`→commercial Coverage), and the referral `Task` (single-Task design, `businessStatus: referral-sent`, no owner yet). I2 = Appointment (carries both candidate procedures), AppointmentResponse, and the updated Task (same id, `owner`=Dr. Maxil for the first time, `businessStatus: appointment-confirmed`). Two self-contained transaction bundles (I1 = 29 resources; I2 = 32 entries, the referral Task included in both its I1 and I2 versions so version history applies in order).

**Design questions resolved during the work:** (A) `Task.businessStatus` = `referral-sent` at I1 → `appointment-confirmed` at I2, flagged in-file as illustrative/consistent with UC02a's use of the same `ode-referral-sub-status` CodeSystem. (D) no-PA response = an informational "no PA required, benefits active" card. (E) `Coverage.type` — **verified against the value set, not guessed:** v3-ActCode `DENTAL` ("dental care policy"); also verified `InsurancePlan.type` = insurance-plan-type `dental`. (G) benefit detail lives in two concrete places — fixed plan structure (annual max, 80%/50%) on `InsurancePlan` (`coverage.benefit.limit` + `plan.generalCost`), and the member/point-in-time-specific values ($1,720 remaining, waiting period waived) narrated on `Coverage`, since in the real world those come back on a PDex/271 benefit response rather than being stored on the Coverage resource. Deliberately did **not** fabricate a `Coverage.costToBeneficiary` copay-type code.

**Verification (Step 7):** all 27 JSON files parse; a scripted cross-reference check confirmed every local reference in both bundles resolves (62 refs in I1, 78 in I2), no duplicate `fullUrl`s; referral-id consistent across ServiceRequest/Task, Task id stable across versions, owner/businessStatus transitions and all timeline dates correct, and no PA/Claim/`preAuthRef` leakage in the referral. Companion guide mirrors UC02a's single-file format (Sections 0–5), scoped to I1/I2 with I3–I5 explicitly marked not-yet-built. **Note on mode:** this build was executed in Claude Code against decisions already framed in the plan; no new design decisions were invented by extrapolation (per Section 7a).

**Not built this session (correctly out of scope):** UC02b Interactions 3–5 (I3 introduces the project's first `Device` resource — the implant; I5 is the commercial-direction claims-sharing proof point with an open scope question about the future crown claim). These remain narrative-only.
