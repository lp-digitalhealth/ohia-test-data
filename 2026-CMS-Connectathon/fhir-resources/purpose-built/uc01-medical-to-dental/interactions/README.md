...
Transactional resources specific to each interaction (as opposed to base/registry resources, which live in `../base/` and the durable registry `fhir-resources/durable/`).

**Library reorg note:** the registry tier was renamed `common/` -> `fhir-resources/durable/` and the use cases moved under `fhir-resources/purpose-built/`. The stale pre-v3.7 `encounters/` folder that used to sit next to this one (and a stray `coverage/` folder) were confirmed superseded and removed.
...
- `cds-hooks-discovery-ibx.json` — CDS Hooks discovery config (NOT a FHIR resource) — hook is `order-sign`. **Moved to `fhir-resources/durable/cds-hooks/`** in the library reorg (it is a durable payer service config, consolidated with the other payers' discovery docs); the interaction-01 bundle still embeds its own inline copy.
