...
|---|---|
| insplan-ibx-pc65ppo.json | Load onto your FHIR server — the plan record CRD evaluates against |
| plandef-ibx-imrt-pa-rule.json | Load onto your FHIR server — CRD entry point |
| lib-ibx-imrt-pa-logic.json | Load onto your FHIR server — CQL logic referenced by the PlanDefinition |
| questionnaire-ibx-imrt-pa-dtr.json | Load onto your FHIR server — served to DTR on launch |
| cds-hooks-discovery-ibx.json | Reference shape for your `/cds-services` discovery document |

1. Load registry resources onto your FHIR server: insplan-ibx-pc65ppo.json, plandef-ibx-imrt-pa-rule.json, lib-ibx-imrt-pa-logic.json, questionnaire-ibx-imrt-pa-dtr.json.
...
| File | Why you need it |
|---|---|
| OMG_O19-dental-referral-request.hl7 | Input: the HL7v2 message your bridge must ingest |
| OMG_O19-dental-referral-request-QA-notes.md | Read first — field-by-field notes on referral ID placement, no `IN1` segment, corrections applied |
...
| File | Why you need it |
|---|---|
| patient-john-smith.json | Load into your system — subject of the IMRT order |
| coverage-john-smith.json | Load into your system — evaluated by CRD when the order fires |
| servicerequest-imrt-order.json | Reference shape for the IMRT order that triggers the `order-sign` CDS Hooks call |
| OMG_O19-dental-referral-request.hl7 | The HL7v2 message your system sends to the bridge — no FHIR production needed for this pathway |

1. Load patient-john-smith.json and coverage-john-smith.json so the order has a subject and coverage to evaluate.
...
4. Separately, send the referral as an `OMG^O19` HL7v2 message (see OMG_O19-dental-referral-request.hl7) to the bridge — **this is HL7v2/C-CDA only; you do not need to produce any FHIR resources for this pathway.**
...
| File | Why you need it |
|---|---|
| task-360x-dental-referral.json | The resource your app queries for referral status — `Task.businessStatus` = `received` after Encounter #1 |
...
- Must be able to send an `OMG^O19` HL7v2 message matching the shape in OMG_O19-dental-referral-request.hl7
...
- Must expose a `/cds-services` discovery document listing an `order-sign` service (see cds-hooks-discovery-ibx.json for the shape)
- Must return a card indicating PA is required and DTR should launch, when queried with an IMRT-coded `ServiceRequest`
- Must be able to serve the DTR `Questionnaire` (questionnaire-ibx-imrt-pa-dtr.json)
...
| endpoint-ibx.json | Endpoint | IBX payer FHIR endpoint |
| endpoint-penndental.json | Endpoint | Penn Dental FHIR endpoint |
| insplan-ibx-pc65ppo.json | InsurancePlan | IBX PC65 PPO — John's insurance plan |
| plandef-ibx-imrt-pa-rule.json | PlanDefinition | IBX IMRT prior auth rule — CRD entry point |
| lib-ibx-imrt-pa-logic.json | Library | CQL logic for IMRT prior auth evaluation |
| questionnaire-ibx-imrt-pa-dtr.json | Questionnaire | DTR questionnaire for IMRT prior auth |
...
| list-john-smith-medications.json | List (ODEMedicationList) | Active medication list — in referral `supportingInfo` |
| medrequest-john-smith-lisinopril.json | MedicationRequest | Lisinopril |
| medrequest-john-smith-atorvastatin.json | MedicationRequest | Atorvastatin |
...
| File | Type | Purpose |
|---|---|---|
| OMG_O19-dental-referral-request.hl7 | HL7v2 PCC-55 | Wire-level referral request (`OMG^O19`) sent by EHR to bridge |
| OMG_O19-dental-referral-request-QA-notes.md | QA notes | Field-by-field verification notes — **read before modifying the `.hl7` file** |
...
The OMG_O19-dental-referral-request.hl7 file is plain pipe-delimited text — readable in any text editor, no special software needed just to look at one. But to validate, transmit, or programmatically process it the way a real interface would, you need one of the following, depending on your role:

**To parse/validate structure only** (e.g., if you're the bridge or EHR checking field positions):
A library, not a full application — HAPI HL7v2 (Java), Python's `hl7` or `hl7apy`, or .NET's NHapi. This is sufficient for confirming the message matches the shape documented in OMG_O19-dental-referral-request-QA-notes.md.
