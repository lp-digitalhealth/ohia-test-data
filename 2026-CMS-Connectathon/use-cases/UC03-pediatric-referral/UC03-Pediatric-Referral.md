...
Dr. Smith recommends that Timothy follow up with a **pediatric dental provider** and creates a structured referral in Epic. The referral is **routed through Connie** — Connecticut's statewide HIE and the central hub for this workflow — to **BeneCare**, the Administrative Services Organization (ASO) that runs the HUSKY dental benefit for the **Connecticut Dental Health Partnership (CTDHP)**. BeneCare handles the network directory, referral coordination, and prior-authorization intake; its Plan-Net directory identifies a participating pediatric dental provider within 20 miles who is accepting new patients enrolled in Husky B. Because this is a referral for a covered evaluation rather than a treatment order, no coverage-requirements discovery (CRD) fires at referral creation — the dental benefit is simply confirmed active; CRD (and, if a planned procedure needs it, DTR/PAS via Gainwell) comes into play at the treatment visit, not here. The directory identifies **Dr. David Watson, DDS**, a pediatric dental provider accepting Husky B, and schedules an appointment — which is available **three months** after the well-child visit.
...
| **Coverage requirements discovery (CRD):** At the treatment visit (not at referral), when Dr. Watson plans the scaling and root planing, dental-benefit coverage/documentation requirements are surfaced. A covered evaluation referral does not trigger CRD. | Da Vinci CRD | Coverage requirements routed/held via Connie at the treatment visit; **D4341 requires prior authorization**, so DTR and PAS are performed downstream by Gainwell (DSS fiscal agent) and the full CRD/DTR/PAS cycle runs at I3. Not exercised at referral (I1). |
...
| **Comprehensive oral evaluation and periodontal risk assessment:** Dr. Watson completes a full evaluation and PRA, confirming periodontitis progression. | US Core / ODE (Under Development) | `Encounter` (in-office, POS 11); `Condition` (K05.211 — aggressive periodontitis, localized, slight); `Observation` (periodontal findings per site, tooth development status); `DiagnosticReport` (PRA). |
...
| **Encounter summary returned to pediatrician — via Connie:** Dr. Watson transmits a structured summary to Dr. Smith at Northeast Medical Group, routed through Connie. | Da Vinci CDex / ODE (Under Development) / Connie HIE | `ClinicalImpression` (encounter summary); `Condition` (K05.211, confirmed); `Procedure` (completed); `CarePlan` (post-treatment oral hygiene, recall frequency); CDex provider-to-provider push routed through Connie to Epic FHIR endpoint at Yale New Haven Health. |
...
| **Da Vinci Coverage Requirements Discovery (CRD)** | Fires at the dental treatment visit (not at the referral) when Dr. Watson plans the scaling and root planing; Connie serves as the routing mechanism and holder of many of these requirements, with DTR/PAS performed downstream by Gainwell (the DSS fiscal agent). **D4341 requires prior authorization**, so the full CRD/DTR/PAS cycle runs at I3. The covered-evaluation referral in I1 does not trigger CRD |
...
| `Condition` | US Core / ODE | E10.9 (Type 1 diabetes, without complications); K05.00 (acute gingivitis, plaque-induced — documented at well-child visit); K05.211 (aggressive periodontitis, localized, slight — confirmed at dental encounter); Z77.22 (tobacco smoke exposure) |
...
7. **Diabetes-periodontal clinical relationship as structured data** — The oral-systemic relationship between Type 1 diabetes and accelerated periodontal disease is the clinical core of this use case. The test validates whether this relationship can be expressed as structured FHIR data — a `Flag` resource to the endocrinologist, a `Condition` reference linking K05.211 to E10.9 as a contributing condition — rather than a clinical note paragraph that cannot be acted on by a receiving system.
...
> **Note on CRD / DTR / PAS / 278:** Coverage-requirements discovery does **not** fire at the I1 referral — that is a referral for a *covered evaluation*, so the benefit is simply confirmed active. **CRD fires at the treatment visit (I3)** when Dr. Watson plans a procedure. **D4341 (scaling and root planing) requires prior authorization** under the HUSKY B benefit — so the full **CRD → DTR → PAS (X12 278)** cycle is exercised at I3: CRD surfaces the PA requirement, and **DTR and PAS are performed downstream by Gainwell** (the DSS fiscal agent), routed through **Connie** with PA intake handled by CTDHP/BeneCare. The resulting `preAuthRef` is carried on the D4341 lines of the I5 claims package. (This resolves the earlier "PA varies by benefit year / open finding" hedge for D4341; D4381 coverage remains a separate open question — see below.)
...
| `D4341` | Periodontal scaling and root planing — four or more teeth per quadrant | Pediatric dental practice / Dr. Watson | Covered; **requires prior authorization** — PA cycle (CRD/DTR/PAS) exercised at the treatment visit (I3); billed as two lines (two quadrants); medical-necessity documentation required for a pediatric patient |
...
| **Periodontal Treatment** | Covered; prior authorization required | D4341 — unusual for pediatric patient; PA (CRD/DTR/PAS) obtained at treatment visit; medical necessity required |
...
| **Code** | K05.211 — Aggressive periodontitis, localized, slight | ICD-10 confirmed dental diagnosis |
...
| **Reason Reference** | K05.211 (aggressive periodontitis, localized, slight); E10.9 (Type 1 diabetes — elevated periodontal risk) | Linked diagnoses |
...
| **Reason Reference** | K05.211 (aggressive periodontitis); E10.9 (diabetes — elevated bacterial risk) | Clinical indication |
...
| **Summary** | Timothy Jones seen 2026-05-28 per referral from Northeast Medical Group. Comprehensive oral evaluation and periodontal risk assessment completed. Gingivitis has progressed to early-stage aggressive periodontitis (K05.211), localized, slight — consistent with accelerated periodontitis trajectory in Type 1 diabetes. Scaling and root planing performed in two quadrants (D4341 ×2). Chlorhexidine chip placed at affected sites (D4381). Oral hygiene education provided to patient and grandmother (D1330); toothbrush, toothpaste, and floss dispensed. Six-month recall recommended. Permanent restoration of partially erupted lower central incisor to be monitored at recall. | Encounter summary |
...
| K05.211 | Aggressive periodontitis, localized, slight | Confirmed at dental encounter by Dr. Watson; transmitted in bidirectional summaries |
...
| `D4341` | Periodontal scaling and root planing — four or more teeth per quadrant | Dr. Watson | Covered; **requires prior authorization** (PA cycle at treatment visit); billed as two lines (two quadrants); medical necessity documentation required for pediatric patient |
